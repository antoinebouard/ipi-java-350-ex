...
public final class EmployeBuilder {
    ...
    public EmployeBuilder but() {
        return anEmploye().withId(id).withNom(nom).withPrenom(prenom).withMatricule(matricule).withDateEmbauche(dateEmbauche).withSalaire(salaire).withPerformance(performance).withTempsPartiel(tempsPartiel);
    }
